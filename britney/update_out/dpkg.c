...
static collpackagelist *get_matching(dpkg_packages *pkgs, deplist *depopts);
...
static int dependency_counts[] = { 2, 2, 1, 0 };
...
char *my_strdup(char *foo) {
	...
	if (!which) {
		...
		DEBUG_ONLY(fprintf(stderr, 
			"Allocated string memblock %d (%lu KiB)\n", 
			stringmemorycount,
			(unsigned long) sizeof(struct stringmemblock) / 1024);)
...
static dpkg_paragraph *read_paragraph( FILE *f ) {
    ...
    if (line == NULL) die("read_paragraph malloc 0:");
 
    result = block_malloc( sizeof(dpkg_paragraph) );
    if (result == NULL) die("read_paragraph malloc 1:");
...
static dpkg_package *new_package(void) {
    ...
    if (result == NULL) die("new_package malloc:");
...
static dpkg_collected_package *new_collected_package(dpkg_package *pkg) {
    ...
    if (result == NULL) die("new_collected_package: malloc:");
...
static dpkg_package *read_package( FILE *f ) {
	...
	if (strcasecmp("Version", e->name) == 0) {
        ...
        if (strcasecmp("Section", e->name) == 0) {
            result->section = my_strdup(e->value);
            if (result->section == NULL)
                die("read_package my_strdup:");
            result->section[strlen(result->section)-1] = '\0';
        }
...
LIST_IMPL(packagenamelist, char*, KEEP(char*), malloc, freesize);
LIST_IMPL(ownedpackagenamelist, char*, KEEP(char*), malloc, freesize);
...
LIST_IMPL(collpackagelist, dpkg_collected_package *, 
	  KEEP(dpkg_collected_package *), malloc, freesize);
...
dpkg_packages *new_packages(char *arch) {
    ...
    if (result == NULL) die("new_packages: malloc:");
...
static char *read_until_char(char **buf, char *end) {
    ...
    if (result == NULL) die("read_until_char malloc:");
...
static dependency *read_dependency(char **buf, char *end) {
    ...
    if (dep == NULL) die("read_dependency malloc 1:");
		                          ...
		                          dpkg_packages *pkgs, dependency *dep)
{
	...
	*colon = '\0';
	...
	if (add) {
	    insert_collpackagelist(addto, vpkg->value.pkg);
	    addto = &(*addto)->next;
	}
    }
...
static collpackagelist *get_matching(dpkg_packages *pkgs, deplist *depopts) {
	...
	addto = get_matching_low(addto, pkgs, depopts->value);
    }
...
static int caninstall(dpkg_packages *pkgs, dpkg_collected_package *cpkg) {
    ...
    conflicts = get_matching(pkgs, cpkg->pkg->conflicts);
...
static void install(dpkg_packages *pkgs, dpkg_collected_package *cpkg) {
    if (cpkg->installed == 0) {
	collpackagelist *conflicts = get_matching(pkgs, cpkg->pkg->conflicts);
	collpackagelist *conf;
	for (conf = conflicts; conf != NULL; conf = conf->next) {
...
satisfieddep *new_satisfieddep(void) {
	satisfieddep *sd = malloc(sizeof(satisfieddep));
	if (!sd) die("new_satisfieddep: malloc:");
	return sd;
}
...
LIST_IMPL(satisfieddeplist, satisfieddep *, free_satisfieddep, malloc, freesize);
...
satisfieddeplist *checkunsatisfiabledeps(dpkg_packages *pkgs, 
	...
	for (; deps != NULL; deps = deps->next) {
		...
		/* for (dep = deps->value; dep != NULL; dep = dep->next) { */
			...
			deppkgs = get_matching(pkgs, deps->value);
...
int checkinstallable(dpkg_packages *pkgs, collpackagelist *instoneof) {
	...
	if (instpkg->installed == 1) {
	    ...
	    /* only expand once */
	    int expansion = I1EXPANDED(pointer) ? 0 : 1;

	    for (i = 0; i < 4; i++) {
		if (!dependency_counts[i]) continue;
                if (dependency_counts[i] == 1) {
                    if (!instpkg->pkg->section ||
                        strcmp(instpkg->pkg->section, "metapackages") != 0)
                        continue;
                }
		for (dep = instpkg->pkg->depends[i];
		     dep != NULL; dep = dep->next)
		{
		    collpackagelist *thisdep = get_matching(pkgs, dep->value);
	...
	for (pointer = last; pointer != NULL; pointer = I1PREV(pointer)) {
		...
		insert_packagenamelist(&I1CUR(pointer)->value->mayaffect,
				       cpkg->pkg->package);
	    }
	    uninstall(pkgs, I1CUR(pointer)->value);
	}
...
static dpkg_source *new_source(dpkg_sources *owner) {
    ...
    if (result == NULL) die("new_source malloc 1:");
    ...
    if (result->packages == NULL) die("new_source malloc 2:");
...
dpkg_source_note *new_source_note(dpkg_source *src, int n_arches) {
    ...
    if (result == NULL) die("new_source_note: malloc 1:");    
    result->source = src;
    result->n_arches = n_arches;
    result->binaries = block_malloc(n_arches * sizeof(packagelist*));
    if (result->binaries == NULL) die("new_source_note: malloc 2:");
    for (i = 0; i < n_arches; i++) {
...
static void save_empty_source_note(dpkg_sources_note *srcsn, dpkg_source *src) {
    ...
    if (srcn == NULL) die("save_empty_source_note: malloc:");
    assert(is_sources_note(srcsn));
